...
import { SalesReportRepositoryStub } from 'test/unit/services/sales-report/stubs';
...
describe('SalesReportService test cases', () => {
    describe('method findByDays test cases', () => {
        it('should return ServerError if SalesReportRepository throws', async () => {
            ...
            const result = await sut.findBydays(7); // Providing a sample argument for 'days'
        ...
        it('should return NotFoundError if no records were found for the provided parameters', async () => {
            ...
            expect(error.message).toBe('Nenhum dado encontrado para os parametros informados');
        });
            ...
            expect(result.value).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({
                        ...
                        salesOrderId: expect.any(String),
                    }),
                ])
            );
        });
